...
pet = ProfilUser()
// Сохраняем нового питомца в JSON
pets := &model.Pets{Pets: []model.Pet{*pet}}
storage.UpdatePetInfo(pets, storage.GetPetsFilePath())
} else {
pet = &existingPets.Pets[0]
// Обновляем сонливость при запуске (если долго не играли)
storage.AutoUpdateSleepiness(pet.Name, storage.GetPetsFilePath())
// Обновляем здоровье при запуске (если долго не играли)
storage.AutoUpdatePetStats(pet.Name, storage.GetPetsFilePath())
// Перезагружаем питомца после обновлений
updatedPets, _ := storage.LoadPetsFromFile(storage.GetPetsFilePath())
if len(updatedPets.Pets) > 0 {
pet = &updatedPets.Pets[0]
}
}
